...
client.connect(("127.0.0.1", 3306)) # SE CONECTA NO SERVIDOR E PORTA
...
window.geometry("550x600")  # Define um tamanho inicial maior para melhor visibilidade

# Barra superior
header = tk.Frame(window, bg="steel blue", height=50)
header.pack(fill=tk.X)

title = tk.Label(header, text="Chatroom", bg="steel blue", fg="white", font=("Arial", 25, "bold"))
title.pack(side=tk.LEFT, padx=10, pady=5)

exit_button = tk.Button(header, text="Exit", command=exit_chat, bg="red", fg="white", font=("Arial", 12, "bold"))
...
chat_display = tk.Text(chat_frame, bg="white", font=("Arial", 12), wrap="word", state=tk.DISABLED)
...
send_button = tk.Button(bottom_frame, text="Enviar", command=send_message, font=("Arial", 12, "bold"), bg="green", fg="white")
